...
para = {"window_size": 0.5, "step_size": 0.2, "structured_file": "./result/messages.log_structured.csv",
        "Log_sequence": './result/messages_sequence.csv'}
...
def load_Log():
    ...
    log_structured['Timestamp'] = pd.to_datetime(
        log_structured.apply(lambda row: f"{current_year}-{int(row['Month']):02d}-{int(row['Day']):02d} {row['Time']}",
                             axis=1))
    # calculate the time interval since the start time
    log_structured["seconds_since"] = (
                log_structured['Timestamp'] - log_structured['Timestamp'][0]).dt.total_seconds().astype(int)
