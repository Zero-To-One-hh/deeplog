...
class Trainer():
    def __init__(self, model, options):
        ...
        if self.sample == 'sliding_window':
            train_logs, train_labels = sliding_window(self.train_data,
                                                  datatype='train',
                                                  window_size=self.window_size,semantics=self.semantics)
            val_logs, val_labels = sliding_window(self.val_data,
                                              datatype='val',
                                              window_size=self.window_size,
                                              sample_ratio=0.001,semantics=self.semantics)
            print('val_labels:', val_labels[:10])
