...
@app.get("/startcollect")
def start_collect():
    if log_collector.get_status():
        return {"code": 500, "message": "日志采集器已经在运行中！", "status": "start"}

    log_collector.start()
    return {"code": 200, "message": "日志采集器启动成功！", "status": "start"}


@app.get("/stopcollect")
def stop_collect():
    if not log_collector.get_status():
        return {"code": 500, "message": "日志采集器已停止或未启动！", "status": "stop"}

    log_collector.stop()
    return {"code": 200, "message": "日志采集器已成功停止！", "status": "stop"}


@app.get("/status")
def get_status():
    status = "start" if log_collector.get_status() else "stop"
    return {"code": 200, "message": "日志采集器状态", "status": status}


def start_fastapi_server():
    import uvicorn
    uvicorn.run(app, host="127.0.0.1", port=8083)  # 监听所有 IP
